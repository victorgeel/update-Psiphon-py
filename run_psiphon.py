...
# Define binary path also relative to the script directory (where build output goes)
...
if not os.path.exists(psiphon_binary_path): 
    ...
    if not build_psiphon_from_clone(): 
...
if not os.path.exists(config_file_path):
     ...
     try:
          # --- MODIFIED DEFAULT CONFIG (with required IDs) ---
          default_config = {
               "LocalSocksProxyPort": 1080,
               "LocalHttpProxyPort": 8080,
               # Add required fields with example/default values
               "PropagationChannelId": "24BCA4EE20BEB92C", # Example from Psiphon README
               "SponsorId": "721AE60D76700F5A"          # Example from Psiphon README
          }
          # --- END MODIFIED DEFAULT CONFIG ---
          with open(config_file_path, 'w') as f:
               json.dump(default_config, f, indent=4)
          # Updated message
          print(f">>> Created default '{config_file_path}' with required IDs. You may edit it if needed.") 
     except Exception as e:
...
if use_proxychains and proxychains_path:
     ...
     print("[Warning] 'use_proxychains' is True, but 'proxychains-ng' not found in PATH. Psiphon will run directly.")
...
try:
    ...
    while True: # Output monitoring loop 
